    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                // Image centered at the top with fixed size
                ...
                // Medicines section (with border)
                sectionWrapper(backgroundColor: Color(hex: "#0D5C8B"), hasBorder: true, borderColor: Color(hex: "#0D5C8B")) {
                    sectionToggle(title: "Medicines", isExpanded: $showCauses) {
                        Text(issue.medicines)
                    }
                }
            }
            .padding()
            ...
            Button(action: {
                ...
                HStack {
                    Text(title)
                        ...
                        .bold()
                        .foregroundColor(
                            title == "Symptoms" ? Color(hex:"#0D5C8B") :
                                title == "Causes" ? Color(hex:"#E2EFFD"):
                                title == "Medicines" ? Color(hex:"#E2EFFD"):
                                title == "Treatments" ? Color(hex:"#0D5C8B"): .primary)
                    Spacer()
...
struct SkinIssuesListView: View {
    ...
    @State private var selectedCategory: String = "All" // Estado para la categoría seleccionada
    
    // Lista de categorías disponibles
    let categories = ["All", "Mild", "Moderate", "Hazardous", "Severe", "Fatal"]
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 20) {
                ...
                // Barra de búsqueda y filtro en HStack
                HStack {
                    // Campo de texto para la búsqueda
                    TextField("Search skin issues...", text: $searchText)
                        .padding(9)
                        .background(Color(.systemGray6))
                        .cornerRadius(10)
                        .padding(.horizontal)
                        .frame(height: 3) // Ajusta la altura de la barra de búsqueda
                    
                    // Spacer para aumentar el espacio entre la búsqueda y el filtro
                    Spacer()
                        .frame(width: 20)
                ...
                // Lista de problemas de la piel filtrada por el texto de búsqueda y la categoría
    ...
    // Propiedad computada para filtrar los problemas de la piel según el texto de búsqueda y la categoría seleccionada
    private var filteredSkinIssues: [SkinIssue] {
        issues.filter { issue in
            // Filtrar por búsqueda en nombre o descripción
            let matchesSearchText = searchText.isEmpty || issue.name.lowercased().contains(searchText.lowercased()) || issue.description.lowercased().contains(searchText.lowercased())
            
            // Filtrar por categoría seleccionada
            let matchesCategory = selectedCategory == "All" || issue.severity == selectedCategory
            
            // Devolver el problema de piel si cumple ambas condiciones
