...
struct SkinIssuesListView: View {
    ...
    @State private var showingCategoryMenu = false // Estado para mostrar el menú de categorías
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 20) {
                ...
                HStack {
                    ...
                    // Botón de filtro con el ícono típico de iPhone
                    Button(action: {
                        showingCategoryMenu.toggle() // Mostrar u ocultar el menú de categorías
                    }) {
                        Image(systemName: "line.horizontal.3.decrease.circle.fill") // Ícono de filtro
                            .font(.title)
                            //.foregroundColor(Color(hex: "#0D5C8B"))
                            .padding(.trailing)
                    }
                }
                .padding(.horizontal) // Espaciado horizontal para todo el HStack
        ...
        .actionSheet(isPresented: $showingCategoryMenu) {
            ActionSheet(title: Text("Filter by Severity"), message: nil, buttons: [
                .default(Text("All")) {
                ...
                .cancel() // Botón para cancelar la selección del filtro
            ])
        }
    }
